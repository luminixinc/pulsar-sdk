...
| Display a single `reference` field (e.g. `AccountId` → Account name) | Schema `referenceTo` → target `nameField` → `read(target, { Id })` — field-tested pattern in `pulsar-metadata`'s `field-display-rules.md`; do **not** use `resolveSOQLFieldPath` for this |
...
- `select` supports full SQLite — including **JOINs across objects** (tables are named by the
  object API name); see the FSL traversal examples in `pulsar-sfs-embedded`'s
  `fsl-data-patterns.md`.

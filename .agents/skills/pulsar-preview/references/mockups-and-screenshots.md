...
```bash
node <this-skill>/scripts/serve.mjs <app-root>    # app-root = parent of mockups/, contains index.html
...
node <skill>/scripts/screenshot.mjs <app-root> mockups/preview.html \
...
node <skill>/scripts/screenshot.mjs <app-root> mockups/account-list-v2.html \
...
## Presenting previews per harness

- **Claude Code**: send the mockup HTML or the PNG into the session so it renders inline —
  this is what remote-control users see on web/mobile. Screenshots (PNG) are the most
  reliable cross-device; static mockup HTML also renders well.
- **Codex CLI / Qwen Code / Gemini CLI**: print the absolute file path(s) plus, for tier 2,
  the serve command and URL to open.

...
- `update(objectName, id, fields, args)` – Update a single Salesforce record, with optional layout-validation behavior.
- `delete(objectName, id)` – Delete a record.
- `select(objectName, whereClause)` – Query records using a simplified filter.
- `soqlQuery(query)` – Query the current Salesforce org using Salesforce Object Query Language.

### Sync Operations
- `syncData(options)` – Initiate a standard, mini, single-object, or push-changes Pulsar sync.
- `syncStatus()` – Retrieve the current Pulsar sync status response.
- `syncRunning()` – Return whether a Pulsar sync is currently running as a boolean.
- `syncInfo()` – Retrieve diagnostic and result information about the most recent sync.
...
### `async update(objectName: string, id: string, fields: object, args?: object): Promise<string>`

Updates a single existing Salesforce record using the Pulsar JSAPI.

The record’s Salesforce `Id` is passed as a separate argument. The SDK adds that value to the request’s `data` object along with the supplied fields.

Only one record can be updated per call.

### Parameters

| Parameter                           | Type                | Required | Description                                                                                                                   |
| ----------------------------------- | ------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------- |
| `objectName`                        | `string`            | ✅        | The API name of the Salesforce SObject to update, such as `"Account"` or `"Contact"`.                                         |
| `id`                                | `string`            | ✅        | The Salesforce Id of the record to update.                                                                                    |
| `fields`                            | `object`            | ✅        | A key-value map containing the fields and values to update. Do not include the record `Id`; pass it using the `id` parameter. |
| `args`                              | `object`            | ❌        | Optional arguments that control update behavior. Defaults to `{}`.                                                            |
| `args.skipLayoutRequiredFieldCheck` | `"TRUE" \| "FALSE"` | ❌        | When `"TRUE"`, skips checking for fields marked as required on the Salesforce layout. The Pulsar API default is `"FALSE"`.    |

### Returns

A `Promise<string>` that resolves to the Salesforce Id of the updated record.

The SDK unwraps the Pulsar bridge response, so the method resolves with the contents of `updateResponse.data` rather than the complete response object.
...
const updatedAccountId = await pulsar.update(
  'Account',
  '001234567891234',
  {
...
The SDK constructs the following Pulsar request:
...
### Example: Skip layout-required field validation

```js
const updatedContactId = await pulsar.update(
  'Contact',
  '003ABC0000001XYZ',
  {
    Phone: '(555) 555-0199'
...
Use this option carefully. It skips Pulsar’s check for fields marked as required on the Salesforce layout. It does not necessarily bypass Salesforce validation rules, field permissions, or other data requirements.

### Success response

The underlying Pulsar response has the following structure:
...
The SDK resolves with only the `data` value:
...
The method throws an error before sending the request when a required argument is invalid or missing.
...
// Throws: Update requires a valid 'id' string.
```

If Pulsar rejects the update, the returned promise rejects with the error supplied by the Pulsar bridge.

Possible causes include:

* A missing or invalid record Id
* An invalid SObject API name
* A field that is not updateable
* Missing layout-required fields
* Salesforce validation-rule failures
* Insufficient object or field permissions
* Other Pulsar or Salesforce data-processing errors

### Concurrency restrictions

Create, update, and delete requests must not run concurrently. Execute these operations sequentially to prevent conflicts.
...
Avoid issuing write operations together with `Promise.all()`:

```js
// Avoid concurrent create, update, or delete requests.
await Promise.all([
  pulsar.update('Account', firstAccountId, firstUpdate),
  pulsar.update('Account', secondAccountId, secondUpdate)
]);
```

### Additional effects

Depending on the features enabled in the Pulsar environment, updating a record may also:

* Trigger PSL Object Triggers for the updated record
* Process applicable Salesforce Validation Rules
* Recalculate and save formula fields
* Recalculate roll-up summary fields on parent records

### Notes

* Only one record can be updated per call.
* Pass the Salesforce record Id using the separate `id` argument.
* The SDK adds the Id to the request’s `data` object.
* Any `Id` property included in `fields` should be ignored or overridden by the explicit `id` argument.
* Only the properties included in `fields` are submitted for update.
* The method resolves to the updated record Id, not the complete updated record.
* Use `read()` after the update when you need the complete current record.
* The current user must have permission to update the record and its fields.
* `skipLayoutRequiredFieldCheck` uses Pulsar’s string boolean format: `"TRUE"` or `"FALSE"`.
...
### async soqlQuery(query: string): Promise<object[]>
Executes a Salesforce Object Query Language (SOQL) query against the currently connected Salesforce org.

Unlike `select()`, which executes a SQLite query against Pulsar’s local offline database, `soqlQuery()` sends a SOQL query to Salesforce. The query must therefore use Salesforce object and field API names and follow SOQL syntax.

The SObject is specified within the query’s `FROM` clause, so this method does not require a separate objectName parameter.
...
### Example with a relationship field
``` js
await pulsar.init();

const contacts = await pulsar.soqlQuery(
...
### Notes
* This method uses Salesforce SOQL syntax, not SQLite syntax.
* Use `select()` when querying records available in Pulsar’s local offline database.
* The query must include its own `FROM` clause; no separate SObject name is passed to the method.
* Only fields included in the SELECT clause are returned.
* Invalid SOQL syntax or Salesforce query errors cause the returned promise to reject.
* The records available to the query are determined by the connected Salesforce org and user session.
...
This is the programmatic equivalent of pressing **Sync Now** in the Pulsar sync status UI. The sync runs in the background, and `syncData()` returns once the sync request has been initiated. It does **not** wait for synchronization to finish.

Applications that need to know when synchronization completes should register a `syncDataFinished` handler **before** calling `syncData()`. A `syncDataUpdate` handler may also be registered to receive progress updates.

When called without options, Pulsar performs a **standard sync**.
...
| Parameter                         | Type       | Required    | Description                                                                          |
| --------------------------------- | ---------- | ----------- | ------------------------------------------------------------------------------------ |
| `options`                         | `object`   | ❌           | Optional configuration controlling the type and behavior of the sync.                |
| `options.miniSyncEnabled`         | `boolean`  | Conditional | Set to `true` to perform a mini sync.                                                |
| `options.miniSyncObjectList`      | `string[]` | Conditional | SObject API names to include. Required when `miniSyncEnabled` is `true`.             |
| `options.singleObjectSyncEnabled` | `boolean`  | Conditional | Set to `true` to perform a single-object sync.                                       |
| `options.rootObjectId`            | `string`   | Conditional | Salesforce Id of the root record. Required when `singleObjectSyncEnabled` is `true`. |
| `options.parentIdFieldList`       | `string[]` | ❌           | Parent reference fields to include during a single-object sync.                      |
| `options.childRelationshipList`   | `string[]` | ❌           | Child relationships to include during a single-object sync.                          |
| `options.childStartDatetime`      | `string`   | ❌           | ISO 8601 datetime used to limit child records based on modification time.            |
| `options.pushChangesSyncEnabled`  | `boolean`  | Conditional | Set to `true` to perform a push-changes-only sync.                                   |
| `options.useComposite`            | `boolean`  | ❌           | Explicitly use the Salesforce Composite API.                                         |
| `options.useCompositeGraph`       | `boolean`  | ❌           | Explicitly use the Salesforce Composite Graph API.                                   |

### Returns

A `Promise<void>` that resolves once Pulsar has accepted and initiated the sync request.

The promise resolving does **not** mean the sync has finished.

Use `registerHandler('syncDataFinished', ...)` to respond to sync completion.

---

### Standard Sync

Calling `syncData()` with no options performs a standard Pulsar sync.

```js
await pulsar.syncData();
```

A standard sync uses Pulsar's normal synchronization behavior and sync windows.

Other sync modes, such as mini syncs, do not modify the sync windows used by standard syncs.

---

### Mini Sync

A mini sync performs a catch-up sync for a subset of the complete synchronized object list.

Enable it with:

* `miniSyncEnabled: true`
* `miniSyncObjectList`: an array of SObject API names to sync

If offline changes are pending for additional object types, Pulsar automatically adds those object types to the mini sync.

The initial mini-sync window is informed by the most recent standard sync. Subsequent mini syncs maintain their own sync window on a per-object basis.
...
This performs a mini sync for `Account` and `Contact`, in addition to any object types Pulsar automatically includes because they have pending offline changes.

---

### Single-Object Sync

A single-object sync synchronizes a specific Salesforce record and can optionally synchronize related parent and child records.

Enable it with:

* `singleObjectSyncEnabled: true`
* `rootObjectId`: the Salesforce Id of the record to synchronize

By default, Pulsar retrieves:

* Parent objects one relationship level above the root record
* Child objects one relationship level below the root record

#### Root object and all applicable parents and children
...
### Controlling Parent Records

Use `parentIdFieldList` to specify which reference fields on the root object should be followed.
...
The values must be Salesforce **field API names** on the root SObject.

If `parentIdFieldList` is omitted, Pulsar includes applicable referenced records for already-synchronized object types.

To suppress parent synchronization, specify an invalid field such as:
...
### Controlling Child Records

Use `childRelationshipList` to specify which child relationships should be synchronized.
...
The values are Salesforce **child relationship names**, not field names.

For custom relationships, the Salesforce UI may display only the base child relationship name. The complete relationship API name generally requires the `__r` suffix.

If `childRelationshipList` is omitted, Pulsar includes applicable child relationships for synchronized object types.

To suppress child synchronization, use:
...
To synchronize only the root record without its parent or child records:
...
### Filtering Child Records by Modification Time

`childStartDatetime` can be used with a single-object sync to limit child relationship queries to records modified since a particular time.

The value must be an ISO 8601 datetime string.
...
When `childStartDatetime` is used, Pulsar queries the specified child relationships for records modified since that datetime.

Existing local child records older than `childStartDatetime` are also cleared from the device.

---

### Push Changes Sync

A push-changes sync bypasses the normal sync process and pushes pending local changes directly to Salesforce.
...
`pushChangesSyncEnabled` must not be combined with `singleObjectSyncEnabled`.

---

### Salesforce API Selection

The API used during synchronization can be explicitly selected.
...
`useComposite` and `useCompositeGraph` must not both be enabled for the same sync.
...
```js
// Invalid: single-object and push-changes modes cannot be combined.
await pulsar.syncData({
  singleObjectSyncEnabled: true,
  rootObjectId: '001234567890123AAA',
  pushChangesSyncEnabled: true
...
```js
// Invalid: Composite and Composite Graph cannot both be selected.
await pulsar.syncData({
  useComposite: true,
  useCompositeGraph: true
});
...
The source API documentation does not specify whether mini sync mode is mutually exclusive with the other specialized sync modes, so applications should not assume an additional restriction beyond those documented above.

### Notes

* `syncData()` returns after the sync request starts; it does not wait for completion.
* Register `syncDataFinished` before starting the sync when completion matters.
* Register `syncDataUpdate` when progress information is needed.
* Calling `syncData()` without options performs a standard sync.
* Mini sync windows are independent from the standard sync window.
* `miniSyncObjectList` is required when `miniSyncEnabled` is `true`.
* `rootObjectId` is required when `singleObjectSyncEnabled` is `true`.
* `singleObjectSyncEnabled` cannot be combined with `pushChangesSyncEnabled`.
* `useComposite` cannot be combined with `useCompositeGraph`.
* Avoid starting another sync from inside a sync lifecycle handler, as this can create unexpected sync loops.
...
Returns information about the current Pulsar sync operation.

This method can be used to determine whether Pulsar is actively performing a sync. It does not start, stop, or otherwise modify the current sync operation.

This differs from `getAutosyncStatus()`, which reports whether automatic syncing is enabled. `syncStatus()` reports whether a sync is running at the moment the request is made.
...
A `Promise<SyncStatusResult>` resolving to an object with the following field:

| Field         | Type                | Description                                                         |
| ------------- | ------------------- | ------------------------------------------------------------------- |
| `syncrunning` | `'TRUE' \| 'FALSE'` | `'TRUE'` when a sync is currently in progress; otherwise `'FALSE'`. |
...
* This method only reports the current sync state.
* It does not wait for a running sync to complete.
...
* The `syncrunning` value is returned as the string `'TRUE'` or `'FALSE'`, not as a JavaScript boolean.
...
Returns diagnostic and result information about the most recent Pulsar sync operation.

This method can be used to inspect whether the most recent sync succeeded, when previous syncs occurred, how many local and server records were processed, and which phases of the sync were performed.

`syncInfo()` reports information about the most recent sync attempt. It does not report whether a sync is currently running. Use `syncStatus()` or `syncRunning()` for the current runtime state.
...
A `Promise<SyncInfoResult>` resolving to an object containing information reported for the most recent Pulsar sync.

All response values are strings. This includes numeric values such as counts and durations and boolean-like values such as `"YES"` and `"NO"`.

| Field                           | Type     | Description                                                          |
| ------------------------------- | -------- | -------------------------------------------------------------------- |
| `lastfailedsync`                | `string` | Timestamp of the most recent failed sync.                            |
| `lastsuccessfulsync`            | `string` | Timestamp of the most recent successful sync.                        |
| `previoussynctime`              | `string` | Timestamp reported by Pulsar for the previous sync.                  |
| `lastsyncdownloadspeed`         | `string` | Download speed reported for the most recent sync.                    |
| `lastsyncduration`              | `string` | Duration reported for the most recent sync.                          |
| `lastsyncsuccess`               | `string` | Whether the most recent sync succeeded, typically `"YES"` or `"NO"`. |
| `lastsyncuploadspeed`           | `string` | Upload speed reported for the most recent sync.                      |
| `localchangespendingcount`      | `string` | Number of local changes still pending after the sync.                |
| `localcreatedcount`             | `string` | Number of locally created records processed.                         |
| `localdeletedcount`             | `string` | Number of locally deleted records processed.                         |
| `localupdatedcount`             | `string` | Number of local update operations processed.                         |
| `localupdateduniquecount`       | `string` | Number of unique locally updated records processed.                  |
| `metadatasyncduration`          | `string` | Duration reported for metadata sync.                                 |
| `metadatasyncperformed`         | `string` | Whether metadata sync was performed.                                 |
| `reachabilitysyncduration`      | `string` | Duration reported for reachability sync.                             |
| `reachabilitysyncperformed`     | `string` | Whether reachability sync was performed.                             |
| `refreshduration`               | `string` | Duration reported for the refresh phase.                             |
| `refreshperformed`              | `string` | Whether a refresh was performed.                                     |
| `schemachanged`                 | `string` | Whether a schema change was detected.                                |
| `serverintegratedcount`         | `string` | Number of server records integrated locally.                         |
| `serverprocessedcount`          | `string` | Number of server records processed.                                  |
| `serverprocessedobjectcountmap` | `string` | Object-specific processed counts reported by Pulsar.                 |
| `syncdomaintype`                | `string` | Domain type used for the sync.                                       |
| `syncgeneration`                | `string` | Sync generation reported by Pulsar.                                  |
| `syncpasscount`                 | `string` | Number of sync passes performed.                                     |
| `syncresumed`                   | `string` | Whether the sync resumed a previous operation.                       |
| `syncwindowtype`                | `string` | Window type used for the sync.                                       |
...
if (info.lastsyncsuccess === 'YES') {
  console.log(
    `Last sync completed successfully in ${info.lastsyncduration}.`
  );
} else {
  console.warn('The most recent sync did not succeed.');
}

console.log('Last successful sync:', info.lastsuccessfulsync);
console.log('Local changes pending:', info.localchangespendingcount);
...
* The method returns information about the most recent sync attempt, not necessarily the most recent successful sync.
* Use `lastsyncsuccess` to determine whether the latest sync succeeded.
* Use `lastsuccessfulsync` and `lastfailedsync` to inspect the recorded sync timestamps.
* Counts and durations are returned as strings and are not converted by the SDK.
* Values such as `"YES"` and `"NO"` are also returned unchanged.
* Use `syncStatus()` or `syncRunning()` to determine whether a sync is currently in progress.
...
| `RelativeFilePath` | `string` | A path relative to the app’s root directory, if applicable. |
